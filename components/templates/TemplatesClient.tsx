...
import { ProjectTemplate } from "@/lib/utils";
import {
  Plus,
  Trash2,
  Rocket,
...
export function TemplatesClient() {
  ...
  async function loadTemplates() {
    try {
      ...
      console.error(error);
      show("error", "Failed to load templates");
    }
  }
    ...
    try {
      ...
      const res = await fetch("/api/projects", {
        ...
        body: JSON.stringify({
          ...
          status: "active",
          priority: selectedTemplate.projectData.priority || "medium",
          startDate: startDate,
          tags: selectedTemplate.tags,
          categories: [selectedTemplate.category],
          // We could copy tasks here if the API supported bulk task creation with project
          // But for now, we'll just create the shell content
        }),
      });
      ...
      if (data.project?.id) {
        // TODO: Copy tasks from template to new project
        // For now just redirect
        router.push(`/projects/${data.project.slug || data.project.uid}`);
      }
    } catch (err) {
  ...
  async function onDelete() {
    ...
    try {
      ...
      show("error", "Failed to delete template");
    }
  }
